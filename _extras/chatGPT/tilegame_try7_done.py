...
class Tile:
    ...
    def draw(self, screen):
        ...
        if self.highlighted:
            pygame.draw.rect(screen, HIGHLIGHT_COLOR, self.rect, 10)
...
class GameManager:
    ...
    def show_sequence_step(self):
        ...
        pygame.display.flip()
        pygame.time.wait(350)  # Short delay before the first tile is shown
        for tile in self.sequence:
            pygame.time.wait(350)
            tile.highlight(screen)
            pygame.time.wait(350)
    ...
    def show_artifact(self):
        ...
        artifact_height = int(SCREEN_HEIGHT * 0.8)
        artifact_width = int(artifact.get_width() * (artifact_height / artifact.get_height()))
        artifact = pygame.transform.scale(artifact, (artifact_width, artifact_height))
    ...
    def show_failure_message(self):
        message = font.render("Incorrect! Wrong Sequence! Try Again.", True, (255, 0, 0))
        screen.fill(BACKGROUND_COLOR)
